 ...
 * Bir konu, yalnızca yeterli sayıda benzersiz doğrulanmış soru olduğunda açılır.
 */
(function(){
'use strict';
const C=window.KPSS_CURRICULUM||{};
const B=Array.isArray(window.KPSS_BANK)?window.KPSS_BANK:[];
const TEST_SIZE=20,TEST_COUNT=5,MIN=TEST_SIZE*TEST_COUNT;
...
function uniqueOriginals(subject,topic){
  const seen=new Set();
  return B.filter(q=>norm(q.subject)===norm(subject)&&norm(q.topic)===norm(topic)&&q.source!=='generated-practice'&&Array.isArray(q.opts)&&q.opts.length===5&&Number.isInteger(Number(q.a))&&Number(q.a)>=0&&Number(q.a)<5&&String(q.text||'').trim()).filter(q=>{
    const key=cleanText(q.text); if(seen.has(key)) return false; seen.add(key); return true;
  });
}
const counts={},incompleteTopics=[],readyTopics=[];
topics.forEach(({subject,topic})=>{
  const originals=uniqueOriginals(subject,topic);
  const key=subject+'|'+topic;
  counts[key]=originals.length;
  if(originals.length<MIN){incompleteTopics.push({subject,topic,available:originals.length,required:MIN,reason:'20 benzersiz soru x 5 test gerekli'});return;}
  readyTopics.push(key);
  originals.slice(0,MIN).forEach((q,i)=>{
    q.testNo=Math.floor(i/TEST_SIZE)+1;
    q.questionNo=i%TEST_SIZE+1;
  });
});
window.KPSS_TOPIC_TESTS={};
topics.forEach(({subject,topic})=>{
  const key=subject+'|'+topic;
  const originals=uniqueOriginals(subject,topic).slice(0,MIN);
  window.KPSS_TOPIC_TESTS[key]=originals.length>=MIN?Array.from({length:TEST_COUNT},(_,i)=>originals.slice(i*TEST_SIZE,(i+1)*TEST_SIZE)):[];
});
...
window.KPSS_CONTENT_AUDIT={subjects:Object.keys(C).length,topics:topics.length,testsPerTopic:TEST_COUNT,questionsPerTest:TEST_SIZE,targetPerTopic:MIN,targetTopicQuestions:topics.length*MIN,actualTopicQuestions:readyTopics.length*MIN,complete:incompleteTopics.length===0,readyTopics,incompleteTopics,counts,version:'CONTENT-LOCK-2026-2'};
window.KPSS_BANK=B;
if(window.KPSS_BANK_INFO){window.KPSS_BANK_INFO.total=B.length;window.KPSS_BANK_INFO.version='CONTENT-LOCK-UNIQUE-5TEST';}
})();
