...
const gesture=write('app.lesson-gesture',fs.readFileSync(path.join(root,'lesson-gesture.js'),'utf8'));
let html=fs.readFileSync(indexPath,'utf8');
html=html.replace(/<script[^>]*src=["'][^"']+["'][^>]*><\/script>/g,'');
html=html.replace(/<script>[^]*?<\/script>/g,'');
html=html.replace('</body>',`<script defer src="${core}"></script><script defer src="${runtime}"></script><script defer src="${ux}"></script><script defer src="${pro}"></script><script defer src="${settings}"></script><script defer src="${lesson}"></script><script defer src="${gesture}"></script></body>`);
fs.writeFileSync(indexPath,html);
console.log('KPSS-Mobil lightweight build: '+core+' + '+runtime+' + '+ux+' + '+pro+' + '+settings+' + '+lesson+' + '+gesture);