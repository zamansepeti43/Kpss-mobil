...
css.textContent=`
/* Premium app chrome: lightweight and interaction-safe */
...
.status>span:last-child{display:none!important}
...
.drawerProgress{height:7px;background:#183047;border-radius:99px;overflow:hidden;margin-top:11px}.drawerProgress i{display:block;height:100%;width:0;background:linear-gradient(90deg,#26d8c0,#3d7cff);border-radius:99px}
...
function updateClock(){
  ...
  if(el)el.textContent=new Intl.DateTimeFormat('tr-TR',{hour:'2-digit',minute:'2-digit',hour12:false}).format(new Date());
}
updateClock();
setInterval(updateClock,30000);

/* Fullscreen is intentionally not bound to document-wide taps.
   The old global double-tap listener could interpret two normal navigation taps
   as a fullscreen gesture and make the mobile page appear locked. */
...
function syncDrawerGoal(){
  ...
  if(source&&target){const m=source.textContent.match(/^\d+/);target.textContent=m?m[0]:'0';}
...
document.addEventListener('click',e=>{
  ...
  if(notify){e.preventDefault();e.stopPropagation();openDrawer();return;}
  if(e.target.closest?.('[data-close-drawer]')){e.preventDefault();closeDrawer();return;}
},{capture:true,passive:false});
document.addEventListener('keydown',e=>{if(e.key==='Escape')closeDrawer()});

/* No DOM observers: navigation/rendering can replace large parts of the page,
   but the menu is created once and drawer state is synchronized only when opened. */
})();
